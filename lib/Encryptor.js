!function(){
	...
	module.exports = new Class({
		...
		, encrypt: function() {
			...
			try {
				config = require(project.git.path+'/travis.js');
			} catch(err) {
				throw new Error('Failed to load the «travis» file from «'+project.git.path+'/travis.js»: '+err).setName('ParserException');
			}
			...
			async.each(Object.keys(config), 
				...
				function(item, next){ 
					this.travis.encrypt(item, function(err, secureData) {
						if(err) throw err;
						items.push(new Buffer(secureData).toString('base64'));
						next();
					}.bind(this));
				}.bind(this), 
		...
		, addToTravisYml: function(items){
			var path = project.git.path+'/.travis.yml';

			fs.readFile(path, function(err, data){
				if (err) {
					console.log(('failed to add to «'+project.git.path+'/.travis.yml» file: '+err).reg);
					this.print(items);
				}
				else {
